...
use RevKeen\Entitlements\Requests\EntitlementsListRequest;
use RevKeen\Types\EntitlementListResponse;
use RevKeen\Entitlements\Requests\EntitlementsCheckRequest;
use RevKeen\Types\EntitlementCheckResponse;
...
class EntitlementsClient
{
     ...
     *
     * @param EntitlementsListRequest $request
     * @param ?array{
     *   baseUrl?: string,
     *   maxRetries?: int,
     *   timeout?: float,
     ...
     * @throws RevkeenApiException
     */
    public function list(EntitlementsListRequest $request, ?array $options = null): EntitlementListResponse
    {
        $options = array_merge($this->options, $options ?? []);
        $query = [];
        $query['customer_id'] = $request->customerId;
        if ($request->includeExpired != null) {
            $query['include_expired'] = $request->includeExpired;
        }
        if ($request->benefitType != null) {
            $query['benefit_type'] = $request->benefitType;
        }
        if ($request->category != null) {
            $query['category'] = $request->category;
        }
        ...
        try {
            ...
            if ($statusCode >= 200 && $statusCode < 400) {
                $json = $response->getBody()->getContents();
                return EntitlementListResponse::fromJson($json);
            }
        } catch (JsonException $e) {
            throw new RevkeenException(message: "Failed to deserialize response: {$e->getMessage()}", previous: $e);
        } catch (RequestException $e) {
     ...
     * Check if a customer has access to a specific benefit by key. This is the primary endpoint for feature gating and licensing checks.
     *
     * @param EntitlementsCheckRequest $request
     * @param ?array{
     *   baseUrl?: string,
     *   maxRetries?: int,
     *   timeout?: float,
     ...
     * @throws RevkeenApiException
     */
    public function check(EntitlementsCheckRequest $request, ?array $options = null): EntitlementCheckResponse
    {
        $options = array_merge($this->options, $options ?? []);
        $query = [];
        $query['customer_id'] = $request->customerId;
        $query['benefit_key'] = $request->benefitKey;
        try {
            $response = $this->client->sendRequest(
                new JsonApiRequest(
                    baseUrl: $options['baseUrl'] ?? $this->client->options['baseUrl'] ?? Environments::Default_->value,
            ...
            if ($statusCode >= 200 && $statusCode < 400) {
                $json = $response->getBody()->getContents();
                return EntitlementCheckResponse::fromJson($json);
            }
        } catch (JsonException $e) {
            throw new RevkeenException(message: "Failed to deserialize response: {$e->getMessage()}", previous: $e);
        } catch (RequestException $e) {
