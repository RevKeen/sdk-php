...
class Price implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    public const START_RULE_IMMEDIATE = 'immediate';
    public const START_RULE_NEXT_ANCHOR = 'next_anchor';
    public const START_RULE_PRORATE = 'prorate';
    public const BILLING_SCHEME_PER_UNIT = 'per_unit';
    public const BILLING_SCHEME_TIERED = 'tiered';
    public const BILLING_SCHEME_PERCENTAGE = 'percentage';
     ...
     * @return string[]
     */
    public function getStartRuleAllowableValues()
    {
        return [
            self::START_RULE_IMMEDIATE,
            self::START_RULE_NEXT_ANCHOR,
            self::START_RULE_PRORATE,
        ];
    }
    ...
    public function getBillingSchemeAllowableValues()
    {
        return [
            ...
            self::BILLING_SCHEME_PERCENTAGE,
        ];
    }
    ...
    */
    ...
    public function listInvalidProperties()
    {
        ...
        $allowedValues = $this->getStartRuleAllowableValues();
        if (!is_null($this->container['start_rule']) && !in_array($this->container['start_rule'], $allowedValues, true)) {
            $invalidProperties[] = sprintf(
                "invalid value '%s' for 'start_rule', must be one of '%s'",
                $this->container['start_rule'],
                implode("', '", $allowedValues)
            );
        }
     ...
     * Sets unit_amount
     *
     * @param int|null $unit_amount Price in minor units (cents). Null when the per-unit rate is sub-penny — see `unit_amount_decimal`.
     *
     * @return self
     */
     ...
     *
     * @param string|null $unit_amount_decimal Exact per-unit rate in minor units as a decimal string (sub-penny safe, e.g. \"0.02\" = 0.02 pence). Set when the rate is not a whole minor unit; mirrors Stripe's `unit_amount_decimal`.
     *
     * @return self
     */
     ...
     * Sets start_rule
     *
     * @param string|null $start_rule First-charge behavior for recurring checkout. `immediate`: full first period now. `next_anchor`: first charge on the billing anchor. `prorate`: prorated stub now, then full charge on the billing anchor.
     *
     * @return self
     */
    public function setStartRule($start_rule)
    {
        ...
        $allowedValues = $this->getStartRuleAllowableValues();
        if (!is_null($start_rule) && !in_array($start_rule, $allowedValues, true)) {
            throw new \InvalidArgumentException(
                sprintf(
                    "Invalid value '%s' for 'start_rule', must be one of '%s'",
                    $start_rule,
     ...
     * Sets name
     *
     * @param string|null $name Customer-facing price name (e.g., 'Morning', 'Full day')
     *
     * @return self
     */
     ...
     * Sets version
     *
     * @param int $version Immutable version for this price row
     *
     * @return self
     */
    public function setVersion($version)
    {
        ...
        if (($version < 1)) {
            throw new \InvalidArgumentException('invalid value for $version when calling Price., must be bigger than or equal to 1.');
        }
     ...
     *
     * @param int|null $percentage_rate_bps Rate in basis points (70 = 0.7%). Present only when `billing_scheme` is `percentage`. Null for `per_unit` and `tiered` prices.
     *
     * @return self
     */
    public function setPercentageRateBps($percentage_rate_bps)
    {
        if (is_null($percentage_rate_bps)) {
            array_push($this->openAPINullablesSetToNull, 'percentage_rate_bps');
        } else {
            $nullablesSetToNull = $this->getOpenAPINullablesSetToNull();
            $index = array_search('percentage_rate_bps', $nullablesSetToNull);
            if ($index !== FALSE) {
                unset($nullablesSetToNull[$index]);
                $this->setOpenAPINullablesSetToNull($nullablesSetToNull);
            }
     ...
     * Sets pay_in_advance
     *
     * @param bool|null $pay_in_advance When true for a metered UBB price, usage events are debited from the customer's prepaid wallet at ingest time.
     *
     * @return self
     */
     ...
     *
     * @param int|null $spending_minimum_cents Minimum minor-unit amount applied to each pay-in-advance usage drawdown when set.
     *
     * @return self
     */
