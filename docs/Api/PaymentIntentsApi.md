...
```php
paymentIntentsRetrieve($id): \RevKeen\Model\PaymentIntent
```
...
try {
    $result = $apiInstance->paymentIntentsRetrieve($id);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling PaymentIntentsApi->paymentIntentsRetrieve: ', $e->getMessage(), PHP_EOL;
}
```
