...
```php
subscriptionsCancelSubscription($id, $cancel_subscription_input): \RevKeen\Model\SubscriptionCancelSubscriptionResponse
```

Cancel subscription

Cancel a subscription either immediately or at the end of the current billing period.  **Modes:** - `immediately`: Status set to \"canceled\", access revoked now - `period_end`: cancelAtPeriodEnd flag set, access continues until period end
...
try {
    $result = $apiInstance->subscriptionsCancelSubscription($id, $cancel_subscription_input);
    print_r($result);
} catch (Exception $e) {
...
```php
subscriptionsRetrieve($id): \RevKeen\Model\SubscriptionRetrieveResponse
```
...
try {
    $result = $apiInstance->subscriptionsRetrieve($id);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling SubscriptionsApi->subscriptionsRetrieve: ', $e->getMessage(), PHP_EOL;
}
```
