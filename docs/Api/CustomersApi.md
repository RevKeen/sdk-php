...
```php
customersRetrieve($id): \RevKeen\Model\CustomerRetrieveResponse
```
...
try {
    $result = $apiInstance->customersRetrieve($id);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling CustomersApi->customersRetrieve: ', $e->getMessage(), PHP_EOL;
}
```
