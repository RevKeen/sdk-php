...
```php
webhookEndpointsRetrieve($id): \RevKeen\Model\WebhookEndpointsRetrieve200Response
```
...
try {
    $result = $apiInstance->webhookEndpointsRetrieve($id);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling WebhookEndpointsApi->webhookEndpointsRetrieve: ', $e->getMessage(), PHP_EOL;
}
```
...
[**\RevKeen\Model\WebhookEndpointsRetrieve200Response**](../Model/WebhookEndpointsRetrieve200Response.md)
...
```php
webhookEndpointsUpdate($id, $webhook_endpoints_update_request): \RevKeen\Model\WebhookEndpointsRetrieve200Response
```
